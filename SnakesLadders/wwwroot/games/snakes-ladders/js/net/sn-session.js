(() => {
  ...
  function getSelectedGameKey() {
    const selected = String(el.gameKey?.value ?? state.createGameKey ?? "").trim();
    return normalizeGameKey(selected, "");
  }

  function buildBoardOptions(boardSize, gameKey = getSelectedGameKey()) {
    if (!gameSupportsBoardOptions(gameKey)) {
      return null;
    }
