(() => {
  ...
  const { el } = root;
  const { gameSupportsBoardOptions } = root.utils;
  ...
  function init() {
    if (el.gameKey) {
      el.gameKey.addEventListener("change", syncAll);
    }
  ...
  function syncAll() {
    const gameKey = root.session?.getSelectedGameKey?.() ??
      ...
      "snakes-ladders";
    const supportsBoardOptions = gameSupportsBoardOptions(gameKey);
    toggleGameSettingsBlock(supportsBoardOptions);
    if (!supportsBoardOptions) {
      hideAllSettings();
      if (el.customRuleOptionsBlock) {
        el.customRuleOptionsBlock.classList.add("hidden");
      }
      if (el.chaosRuleInfoBlock) {
        el.chaosRuleInfoBlock.classList.add("hidden");
      }
      return;
    }
  ...
  function toggleGameSettingsBlock(visible) {
    if (!el.snakesLaddersSettings) {
      return;
    }

    el.snakesLaddersSettings.classList.toggle("hidden", !visible);
    for (const input of el.snakesLaddersSettings.querySelectorAll(
