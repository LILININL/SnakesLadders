(() => {
  ...
  function pointAtPath(pathData, t) {
    ...
    if (pathData.kind === "snake") {
      return cubicPoint(pathData.from, pathData.c1, pathData.c2, pathData.to, progress);
    }
  ...
  function angleAtPath(pathData, t) {
    const progress = clamp(t, 0, 1);
    if (pathData.kind === "snake") {
      const tangent = cubicTangent(pathData.from, pathData.c1, pathData.c2, pathData.to, progress);
      return Math.atan2(tangent.y, tangent.x) * (180 / Math.PI);
    }

    return Math.atan2(pathData.to.y - pathData.from.y, pathData.to.x - pathData.from.x) * (180 / Math.PI);
  }
